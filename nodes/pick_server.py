...
class action_server():
    ...
    def __init__(self):
        ...
        self.a_server = actionlib.SimpleActionServer("pick_as", robot_geoAction, execute_cb = self.execute_cb, auto_start = False)
        self.a_server.start()
        
        #Subscribe to /move_status from move_to_goal_prim.py
        self.move_status_sub = rospy.Subscriber("/move_status", Bool, self.grasp, queue_size = 1)
    ...
    def execute_cb(self, goal):
        ...
        print("Pick server created")
        
        if action_server.goal_action == 'pick':
            print('PICK action to be executed')
    ...
    def grasp(self, msg):
        ...
        if (msg.data == True and action_server.goal_action == 'pick'):
            if (not (action_server.gripper_content) and action_server.obj_reach):
                print("Object has to be GRASPED.")
                subaction_pub.publish("grasp")
                time.sleep(2)
            else:
                if (action_server.gripper_content):
        ...
        action_server.feedback.movement = 'Object PICKED!'
